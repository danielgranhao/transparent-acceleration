...
int func_process(pid_t pid) {
  ...
  // set up our registers with the args to the injected function
  // set up our registers with the args to the injected function
  // set up our registers with the args to the injected function
  // set up our registers with the args to the injected function
...
for x in range(data["callingOrder"]["nTotal"]):
	cog.outl("newregs.%s = oldregs.%s;" % ( regs[x], data["callingOrder"]["order"][x]))
]]]*/
newregs.rdi = oldregs.rdx;
newregs.rsi = oldregs.rcx;
newregs.rdx = oldregs.rsi;
newregs.rcx = oldregs.rdi;
//[[[end]]]
